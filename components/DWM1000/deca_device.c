 ...
 * 0xDECA0130).  Then it does any initial once only device configurations needed
 * for use and initialises as necessary any static data items belonging to this
 * low-level driver.
 *
 * NOTES:
 * 1.this function needs to be run before dwt_configuresleep, also the SPI
 * frequency has to be < 3MHz 2.it also reads and applies LDO tune and crystal
 * trim values from OTP memory
 *
 * input parameters
 * @param config    -   specifies what configuration to load
 *                  DWT_LOADUCODE     0x1 - load the LDE microcode from ROM -
 * enabled accurate RX timestamp DWT_LOADNONE      0x0 - do not load any values
 * from OTP memory
 *
 * output parameters
 *
 ...
 * byte CRC. Note: this is the length of TX message (including the 2 byte CRC) -
 * max is 1023 standard PHR mode allows up to 127 bytes
 *                         if > 127 is programmed, DWT_PHRMODE_EXT needs to be
 * set in the phrMode configuration see dwt_configure function
 ...
 * programmed, DWT_PHRMODE_EXT needs to be set in
 * the phrMode configuration see dwt_configure function
 ...
 * to signify that the sub-index address follows the register index byte b. set
 * bit-7 (or with 0x80) for write operation c. if extended sub address
 * index is used (i.e. if index > 127) set bit-7 of the first sub-index
 * byte following the first header byte
 *
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int IRAM_ATTR dwt_writetodevice(uint16 recordNumber, uint16 index,
  ...
  if (index == 0)  // For index of 0, no sub-index is required
  {
    ...
    header[cnt++] = 0xC0 | recordNumber;  // Bit-7 is WRITE operation, bit-6
                                          // one=sub-address follows, bits 5-0
                                          // is reg file id
 ...
 * returns DWT_SUCCESS for success, or DWT_ERROR for error
 */
int IRAM_ATTR dwt_readfromdevice(uint16 recordNumber, uint16 index,
 ...
 */
uint32 IRAM_ATTR dwt_read32bitoffsetreg(int regFileID, int regOffset) {
 ...
 * 0x004   beacon frames allowed DWT_FF_DATA_EN          0x008   data
 * frames allowed DWT_FF_ACK_EN           0x010   ack frames allowed
 ...
 * pump). Read margin is used to stress the read back from the programmed bit.
 * In normal operation this is relaxed.
 *
 * input parameters
 * @param mode - "0" : Reset all to 0x0:           MRA=0x0000, MRB=0x0000,
 * MR=0x0000 "1" : Set for inital programming: MRA=0x9220, MRB=0x000E, MR=0x1024
 *               "2" : Set for soak programming:   MRA=0x9220, MRB=0x0003,
 * MR=0x1824 "3" : High Vpp:                   MRA=0x9220, MRB=0x004E, MR=0x1824
 *               "4" : Low Read Margin:            MRA=0x0000, MRB=0x0003,
 * MR=0x0000 "5" : Array Clean:                MRA=0x0049, MRB=0x0003, MR=0x0024
 ...
 * to 0. VNM Charge pump needs to be enabled (see _dwt_otpsetmrregs) Note the
 * address is only 11 bits long.
 *
 * input parameters
 * @param address - address to read at
 *
 ...
 * The DW1000 will go so SLEEP straight after this if the DWT_SLP_EN has been
 * set.
 *
 * input parameters
 *
 ...
 * dwt_configuredeepsleep should be called first to configure the sleep and
 ...
 * load operating parameter set on wakeup DWT_CONFIG       0x0040 -
 * download the AON array into the HIF (configuration download) DWT_LOADEUI
 * 0x0008 DWT_GOTORX       0x0002 DWT_TANDV        0x0001
 *
 ...
 * for at least 500us. To define the length depending on the time one wants to
 * hold the chip select line low, use the following formula:
 *
 *      length (bytes) = time (s) * byte_rate (Hz)
 *
 * where fastest byte_rate is spi_rate (Hz) / 8 if the SPI is sending the bytes
 * back-to-back. To save time and power, a system designer could determine
 * byte_rate value more precisely.
 *
...
int dwt_spicswakeup(uint8 *buff, uint16 length) {
      ...
      DWT_DEVICE_ID)  // Device was in deep sleep (the first read fails)
  {
    ...
    // status bit !!!) NOTE: Polling of the STATUS register is not possible
 ...
 * table for 64 length preamble configuration DWT_OPSET_TIGHT = 0x1 - load the
 * operational parameter set table for tight xtal offsets
 * (<1ppm) DWT_OPSET_DEFLT = 0x2 - load the default operational parameter set
 * table (this is loaded from reset)
 *
 * output parameters
 *
 * no return value
...
uint8 dwt_checkIRQ(void) {
   ...
   * @fn dwt_isr()
   *
   * @brief This is the devices interrupt handler function, it will
   * process/report status events Notes:  In PC based system using (Cheetah or
   * ARM) USB to SPI converter there can be no interrupts, however we still need
   * something to take the place of it and operate in a polled way. In an
   * embedded system this function should be configured to launch on an
   * interrupt, then it will process the interrupt trigger event and call a TX
   * or RX call-back function depending on whether the event is a TX or
   * RX event. The TX call-back will be called when a frame has been sent and
   * the RX call-back when a frame has been received.
   *
   * input parameters
   *
   * output parameters
...
void dwt_isr(void)  // Assume interrupt can supply context
{
  ...
  //     function, notifying the application that ACK req is set. If using
  //     auto-ACK, the AAT indicates that ACK frame transmission is in progress.
  //     Once the ACK has been sent the TXFRS bit will be set and TX event
  //     triggered. If the auto-ACK is not enabled, the application can
  //     format/configure and start transmission of its own ACK frame.
  //
  ...
  if ((status & SYS_STATUS_LDEDONE) && (dw1000local.dblbuffon == 0)) {
    if ((status &
      ...
      // enable RX, We can get into here before the TX frame done has been
      // processed, when we are polling (i.e. slow to process the TX)
  ...
  if (status & SYS_STATUS_RXFCG)  // Receiver FCS Good
  {
    if (status & SYS_STATUS_LDEDONE)  // LDE done/finished
    {
      ...
      // should be discarded Read frame info and other registers and check for
      // overflow again If overflow set then discard both frames...
                                       ...
                                       // pointers point to the same buffer
      {
        ...
        // contains the older frame e.g. seq. num = x) will be corrupted with
        // the latest frame (seq. num = x + 2) data, both the host and IC are
        // pointing to buffer A We are going to discard this frame - turn off
        // transceiver and reset receiver
        dwt_forcetrxoff();
      ...
      } else  // No overrun condition - proceed to process the frame
      {
        ...
        // FC is not set
        if ((status &
             SYS_STATUS_AAT)  // AAT bit is set (ACK has been requested)
            &&
        ...
        if (dw1000local.dblbuffon == 0)  // If no double buffering
        {
          ...
          // enabled or TX is started New events can trigger and give rise to
          // new interrupts Call the RX call-back function to process the RX
          // event
          if (dw1000local.dwt_rxcallback != NULL) {
            dw1000local.dwt_rxcallback(&dw1000local.cdata);
          }
        } else  // Double buffer
        {
          ...
          // before starting RX Read again because the status could have changed
          // since the interrupt was triggered
               ...
               << 1))  // Host Side Receive Buffer Pointer
          {
            ...
            dwt_write32bitreg(SYS_STATUS_ID, clear);  // Write status register
                                                      // to clear event bits we
                                                      // have seen
          }
          ...
          // guarantee the last frame's data was not corrupted If no overrun all
          // is good, toggle the pointer
          if (dwt_checkoverrun() == 0) {
            ...
            // other buffer) and the 2nd's PHR good set the overrun flag Due to
            // a receiver bug, which cannot guarantee the last frame's data was
            // not corrupted need to reset receiver and discard any new data
            dwt_forcetrxoff();
      ...
      if (status & SYS_STATUS_TXFRS)  // Transmit Frame Sent
  {
    ...
    // or TX is started New events can trigger and give rise to new interrupts
    if (dw1000local.cdata.aatset) {
      ...
      if (dw1000local.dblbuffon == 0)  // If not double buffered
      {
                                    ...
                                    // start command
        {
          ...
          // requested it the receiver will be re-enabled, so issue a TRXOFF
          // command to disable and prevent any unexpected interrupts
          dwt_forcetrxoff();
        }
      }
    }
 ...
 * automatically turned on after TX is done) if 3 delayed TX (response expected
 * - so the receiver will be automatically turned on after TX is
 * done)
 *
 * output parameters
 *
 ...
 * converters. Note on Temperature: the temperature value needs to be converted
 * to give the real temperature the formula is: 1.13 * reading - 113.0 Note on
 * Voltage: the voltage value needs to be converted to give the real voltage the
 * formula is: 0.0057 * reading + 2.3
 *
 * NB: To correctly read the temperature this read should be done with xtal
 * clock however that means that the receiver will be switched off, if receiver
 * needs to be on then the timer is used to make sure the value is stable before
 * reading
 *
 * input parameters:
