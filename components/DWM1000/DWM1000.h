...
template <typename T>
void little_endian(uint8_t* dst, T v) {
...
template <typename T>
void little_endian(T& v, uint8_t* src) {
  v = 0;
...
class Timeout {
 public:
  ...
  void reset() { _nextTimeout = Sys::millis() + _interval; }
  bool expired() { return Sys::millis() > _nextTimeout; };
};
...
class DWM1000 {
 public:
  ...
  typedef enum { RCV_ANY = H("RCV_ANY"), RCV_RESP = H("SND_POLL") } State;
