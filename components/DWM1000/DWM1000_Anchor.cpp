...
#define TX_ANT_DLY \
  16436 /* Default antenna delay values for 64 MHz PRF. See NOTE 1 below. */
...
/* This is the delay from Frame RX timestamp to TX reply timestamp used for
 * calculating/setting the DW1000's delayed TX function. This includes the frame
 * length of approximately 2.46 ms with above configuration. */
 ...
 * reader can examine it at a breakpoint. */
static double tof;
...
void DWM1000_Anchor::calcFinalMsg() {
  ...
  /* Compute time of flight. 32-bit subtractions give correct answers even if
   * clock has wrapped. See NOTE 10 below. */
  ...
  while (true) {
    ...
    if (signal->event == DWT_SIG_RX_OKAY) {
      ...
      if (ft == FT_POLL && _dwmMsg.getDst() == _shortAddress) {
        ...
        //                   INFO("resp %X :
        //                   %d",_respMsg.getDst(),_respMsg.sequence);
        if (sendRespMsg() == 0) {
          goto WAIT_FINAL;
        };
        //                   WARN("FAIL send resp");
